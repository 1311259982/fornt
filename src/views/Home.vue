...
const fetchLatestArticles = async () => {
  try {
    ...
    // Assuming the API returns data in a specific structure, e.g., res.data.content
    // Adjust this based on your actual API response.
    latestArticles.value = res.data.content || res.data || []
  } catch (error) {
    console.error('获取最新文章失败', error)
    latestArticles.value = [] // Ensure it's an empty array on error
  }
}
